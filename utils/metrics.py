...
def generate_random_images(height, width):
    # 第一张图片：均值为0.5，方差为0.1的高斯噪声
    img1 = np.random.normal(loc=0.5, scale=0.1, size=(height, width, 3)).clip(0, 1)

    # 第二张图片：均值为0.2，方差为0.1的高斯噪声
    img2 = np.random.normal(loc=0.2, scale=0.1, size=(height, width, 3)).clip(0, 1)
...
if __name__ == '__main__':
    # 生成图片
    img1, img2 = generate_random_images(28, 28)
    ...
    # 保存图片以便查看
    cv2.imwrite('random_image1.png', img1)
    cv2.imwrite('random_image2.png', img2)